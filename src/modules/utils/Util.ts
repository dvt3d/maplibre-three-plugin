...
class Util {
   ...
   */
  static getViewInfo(transform: ITransform, center: number[] | string, boundingSize: Vector3): { center: [number, number], cameraHeight: number, zoom: number } {
   ...
   */
  static getHeightByZoom(transform: ITransform, zoom: number, lat: number, pitch: number) {
    const pixelAltitude = Math.abs(
   ...
   */
  static getZoomByHeight(transform: ITransform, height: number, lat: number, pitch: number) {
    const pixelAltitude = Math.abs(
      Math.cos(pitch * DEG2RAD) * transform.cameraToCenterDistance,
    )
    const metersInWorldAtLat
