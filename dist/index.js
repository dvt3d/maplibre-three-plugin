...
var Util = class {
  ...
  static getZoomByHeight(transform, height) {
    ...
    const metersInWorldAtLat = EARTH_CIRCUMFERENCE * Math.abs(Math.cos(transform.center.lat * DEG2RAD));
...
var MapScene = class {
  ...
  getViewPosition() {
    return {
      position: [
        ...
        Util_default.getHeightByZoom(this._map.transform, this._map.transform.zoom)
      ],
  ...
  flyTo(target, completed = null, duration = 3) {
    if (target && target.position) {
      ...
      const viewInfo = Util_default.getViewInfoByBoundingSize(
        this._map.transform,
  ...
  flyToPosition(position, hpr = [0, 0, 0], completed = null, duration = 3) {
    ...
    this._map.flyTo({
      ...
      zoom: Util_default.getZoomByHeight(this._map.transform, position[2]),
      bearing: hpr[0],
      pitch: hpr[1],
      duration: duration * 1e3
    });
